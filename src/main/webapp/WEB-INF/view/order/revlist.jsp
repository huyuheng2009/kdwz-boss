	    ...
	    var data1 = tmjs.clist;
        	...
        	$('#exportData').click(function(){
        	 ...
        	 $("#config").click(function(){
         		$.dialog({lock: true,title:'设置',drag: true,width:600,height:600,resize: false,max: false,content: 'url:${ctx}/tabelFiled/tableFiledSelect?tab=order_revlist&layout=no',close: function(){
              	 }});
         	});
...
<div class="search">
 ...
 <div class="soso">
    ...
    <form:form id="trans" action="${ctx}/order/revlist" method="post" >
     <input type="hidden" name="serviceName" value=""/>
          ...
          <%--   <li class="soso_li">运单号： <input type="text" value="${params['orderNo']}" placeholder="请输入运单号" name="orderNo"></input> 
           ...
           <%--   <li>是否有误：<select name="erred" >
			  ...
			  <option value="2" ${params['erred'] eq '2'?'selected':''} >否</option>
			    ...
			    <li class="soso_li">月结客户：<input type="text" value="${params['monthSettleNo']}" id="monthSettleNo" name="monthSettleNo" style="width:101px" ></input></li>
		
            <li class="search_cont_input">派件员工：<input type="text" value="${params['courierNo']}" name="courierNo" id="courier" style="width:101px"></input></li>
      <li>派件网点：<select id="sendSno" name="sendSno" style="width: 110px">
            ...
            <li>审核状态：<select name="examineStatus" style="width: 110px">
             ...
             <li style="margin-left:15px;">付款方式：<select name="payType" style="width: 110px">
                <option value="">--全部--</option>
  ...
  <div class="tableble_search ">
        <div class="operator">
	      ...
	      <div class="search_new"><input style="width: 100px;" class="sear_butt" type="submit"  value="签收单导出"  onclick="sendExport();"/> </div>
        ...
        <c:forEach items="${list.list}" var="item" varStatus="status">
		          	...
		          	<c:if test="${taf.isShow==1 }">
		          		<td align="center">
			          		...
			          		<c:choose> 
			          			...
			          			<c:when test="${k=='sign_examine_status' }">
			          				<c:if test="${item.sign_examine_status ne 'PASS'}"><span style="color: orange;">未审核</span></font></c:if>
                  					<c:if test="${item.sign_examine_status eq 'PASS'}">已审核</font></c:if>
			          			</c:when>
          ...
          <td>${item.monthSname}</td>  
           ...
           <td>${item.item_weight}</td>
            ...
            <td>${item.rece_no}</td>   
            <td>${item.for_no}</td>  
             <td><c:if test="${item.sign_examine_status ne 'PASS'}"><span style="color: orange;">未审核</span></font></c:if>
                  <c:if test="${item.sign_examine_status eq 'PASS'}">已审核</font></c:if></td>
              <td align="center">
                   <c:if test="${item.deled!=1}">
                  <a href="javascript:note('${item.id}','${item.lgc_order_no}');">备注</a>
                       |
