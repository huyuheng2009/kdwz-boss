...
<script type="text/javascript">
	$(function(){
		trHover('t2');
		 $('#add').click(function(){
			 $.dialog({lock: true,title:'新增优惠类型',drag: false,width:500,height:240,resize: false,max: false,content: 'url:${ctx}/mobile/msedit?layout=no',close: function(){
				 location.reload();
		  	 }});
		 });
	...
	function edit(id){
		 $.dialog({lock: true,title:'编辑优惠类型',drag: false,width:500,height:240,resize: false,max: false,content: 'url:${ctx}/mobile/msedit?id='+id+'&layout=no',close: function(){
			 location.reload();
	  	 }});
	}
...
<div class="tableble_search">
    ...
    <form:form id="trans" action="${ctx}/mobile/mstype" method="get">
    	<ul>
	...
	<table width="100%" cellspacing="0" class="t2" id="t2">
        ...
        <c:forEach items="${list.list}" var="item" varStatus="status">
            <tr class="${status.count % 2 == 0 ? 'a1' : ''}">
                ...
                <td>${item.name}</td>
                <td>应收款为本月产生款项的${item.discount}%</td>
                <td>${item.note}</td>
