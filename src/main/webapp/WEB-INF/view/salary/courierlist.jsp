...
<script>
			function importSubstation() {
				...
				$("form:first input[name=serviceName]").val('tc');
...
//                });
			...
			$.dialog({
				...
				resize: false,
				title: '业务员提成编辑',
				content: "url:<%=request.getContextPath()%>/salary/compile?layout=no&courierNo="+e,
				lock: true	  				
   			});	   
			}
        </script>
...
<body>
	<div class=" soso">
		...
		<!--按钮左右结构-->
		<div class="soso_left">
			<form id="myform" action="cclist" method="post">
				<div class="soso_li">
			...
			<table class="ta_ta" width="100%" cellpadding="0" cellspacing="0"
				...
				<c:forEach items="${list.list}" var="item" varStatus="status">
					<tr>
						...
						<td>${item.phone}</td>
						<td>${item.takeTcf}</td>
						<td><fmt:formatNumber value="${item.takeTcw}" pattern="0.00"/> </td>
						<td><fmt:formatNumber value="${item.takeTcc}" pattern="0.00"/> </td>
						<td>${item.sendTcf}</td>
						<td><fmt:formatNumber value="${item.sendTcw}" pattern="0.00"/></td>
						<td><fmt:formatNumber value="${item.sendTcc}" pattern="0.00"/></td>
						<td>${item.forTcf}</td>
						<td><fmt:formatNumber value="${item.forTcw}"  pattern="0.00"/></td>
						<td><fmt:formatNumber value="${item.forTcc}"  pattern="0.00"/></td>						
						<td><a href="#" onclick="compile('${item.courierNo}')">编辑</a></td>		
					</tr>
				</c:forEach>
		</table>
		</div>
