...
<head>
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $('#sendscan_btn').click(function(){
           	 $.dialog({lock: true,title:'出站扫描',drag: true,width:1000,height:550,resize: false,max: false,content: 'url:${ctx}/scan/sendscan?layout=no',close: function(){
            	}});
          	}); 
...
<div class="tableble_search">
    ...
    <form:form id="trans" action="${ctx}/scan/solist" method="get">
     <input type="hidden" name="serviceName" value=""/>
        <ul>
            <li>运单号：<input type="text" value="${params['lgcOrderNo']}" placeholder="物流单号" name="lgcOrderNo"></input></li>
			 ...
			 <li>运单状态：<select name="orderStatus" style="width: 110px">
  ...
  <div class="operator">
	     <div class="search_new"> <input class="button input_text  big gray_flat" type="submit" value="刷新" id="reload_btn"/> </div>
	      ...
	      <div class="search_new"> <input style="width: 100px;" class="button input_text  big gray_flat" type="submit" value="出站扫描" id="sendscan_btn"/> </div>
	       </shiro:hasPermission>
	    </div>
   </div>   <!-- tableble_search end  -->   
</div>
...
<div class="tbdata">
    ...
    <table width="100%" cellspacing="0" class="t2" id="t2">
        ...
        <tr>
            ...
            <th  align="center" >当前站</th>
            <th  align="center" >当前站名称</th>
            <th  align="center" >上一站</th>
            <th  align="center" >上一站名称</th>
            <th  align="center" >进站扫描员编号</th>
            <th  align="center" >进站扫描员名称</th>
            <th  align="center" >下一站</th>
            <th  align="center" >下一站名称</th>
            <th  align="center" >出站扫描员编号</th>
            <th  align="center" >出站扫描员名称</th>
        ...
        <c:forEach items="${list.list}" var="item" varStatus="status">
            <tr class="${status.count % 2 == 0 ? 'a1' : ''}">
               ...
               <td><c:if test="${params['orderStatus'] ne 'INIT' }">${item.pre_no}</c:if></td>
               <td><c:if test="${params['orderStatus'] ne 'INIT' }"><u:dict name="${item.pre_type}" key="${item.pre_no}" /></c:if></td>
               <td><c:if test="${params['orderStatus'] ne 'INIT' }">${item.scan_ino}</c:if></td>
               <td><c:if test="${params['orderStatus'] ne 'INIT' }">${item.scan_iname}</c:if></td>
               <td>${item.next_no}</td>
               <td><c:if test="${params['orderStatus'] eq 'INIT' }"><span  style="color: red;"></c:if><u:dict name="${item.next_type}" key="${item.next_no}" /></span></td>
               <td><c:if test="${params['orderStatus'] ne 'INIT' }">${item.scan_ono}</c:if></td>
               <td><c:if test="${params['orderStatus'] ne 'INIT' }">${item.scan_oname}</c:if></td>
               <td><fmt:formatDate value="${item.create_time}" type="both"/></td>
                <td><c:if test="${params['orderStatus'] eq 'INIT' }"><span  style="color: red;">代收件</span></c:if>
                    <c:if test="${params['orderStatus'] eq 'REV' }"><span style="color:orange;">未发送<span></c:if>
                    <c:if test="${params['orderStatus'] eq 'SEND' }">已完成</c:if></td>
                <td align="center">
