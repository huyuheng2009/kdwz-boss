...
<div class="tableble_search">
    ...
    <form:form id="trans" action="${ctx}/scan/clist" method="get">
        ...
        <ul>
            ...
            <li>快递员：<input type="text" value="${params['courierNo']}" placeholder="快递员编号" name="courierNo" id="courierNo"></input></li>
			 ...
			 <li>运单状态：<select name="orderStatus" style="width: 110px">
                ...
                <option value="1" ${params['orderStatus'] eq '1'?'selected':''}>待收件</option>
                <option value="2" ${params['orderStatus'] eq '2'?'selected':''}>已收件</option>
             </select></li>
    ...
    <table width="100%" cellspacing="0" class="t2" id="t2">
        ...
        <c:forEach items="${list.list}" var="item" varStatus="status">
            <tr class="${status.count % 2 == 0 ? 'a1' : ''}">
               ...
               <td><c:if test="${item.status==1 }"><span style="color:orange;">待收件<span></c:if>
                    <c:if test="${item.status!=1 }">已收件</c:if></td>
                <td align="center">
