...
//			}
			function del() {
				...
				$.dialog.confirm('您真的确定要删除订单吗？', function() {
					location.href = "<%=request.getContextPath()%>/batchOrder/delOrderAddrImport?ids=" + ids.join(",");
				}, function() {
			...
			function cleanAll() {
				$.dialog.confirm('您真的确定要清空吗？', function() {
					location.href = "<%=request.getContextPath()%>/batchOrder/cleanOrderAddrImport?uid=${uid}";
				}, function() {
    ...
    <body style="background: #fff;">
        <div class="table">
            <div class="ta_header">
                <form action="queryOrderAddrImport?layout=no" method="post">
...
<!--                    <a href="javascript:void(0);" class="button_2" onclick="addAddr('<%=request.getContextPath()%>/batchOrder/addOrderAddrImportPage?layout=no');">新增</a>					-->
                </div>
            </div>
            <div class="ta_table">
                <table class="ta_ta" width="100%" cellpadding="0" cellspacing="0" border="0">
                    ...
                    <c:forEach items="${list}" var="t" varStatus="s">
                        <tr>
                            ...
                            <td>${t.lgcOrderNo}</td>
                            <td>${t.orderNote}</td>
                            <td><a href="javascript:void(0);" onclick="addAddr('<%=request.getContextPath()%>/batchOrder/addOrderAddrImportPage?id=${t.id}&layout=no')">修改</a>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="singleDel('<%=request.getContextPath()%>/batchOrder/delOrderAddrImport?ids=${t.id}')">删除</a></td>        
                        </tr>	
                    </c:forEach>
                </table>
