...
$(function () {
    ...
    var substationList = ${courierList};
	var cres = [];
	  var sres = [];
	  $.each(substationList, function(i, item) {
			sres[i]=item.user_name.replace(/\ /g,"")+'('+item.real_name.replace(/\ /g,"")+')';
	   });
  ...
  $jqq("#inCourierNo").autocomplete(cres, {
		minChars: 0,
		max: 12,
		autoFill: true,
		mustMatch: false,
		...
		if(data[0].indexOf(')')>-1){			
			 $("#inCourierNo").val(data[0].substring(0,data[0].indexOf('('))) ;
			 $("#ctips").html(data[0].substring(data[0].indexOf('(')+1,data[0].indexOf(')')));
		       } 
	}); 	
  $jqq("#outCourierNo").autocomplete(sres, {
		minChars: 0,
		max: 12,
		autoFill: true,
		mustMatch: false,
...
<body>
	<div class="table" >
		<div class="ta_header">
			<form action="inAndOut?layout=no" method="post">
				<div class="ta_h_search"  style="overflow:hidden">
			
				
					进站扫描员 <input class="put_cont" type="text" name="inCourierNo" id = "inCourierNo"	
							placeholder="" value="${param.inCourierNo}">
				...
				<input type="hidden" name="serviceName" value="inAndOut" />
			...
			<table class="ta_ta" width="100%" cellpadding="0" cellspacing="0"
				...
				<thead>
					<tr>
						...
						<th width="50" align="center">到站扫描员名称</th>
						<th width="30" align="center">出站扫描时间</th>
				...
				<tbody>	
					<c:forEach items="${list.list}" var="item" varStatus="status">
						<tr class="${status.count % 2 == 0 ? 'a1' : ''}">
					 	...
					 	<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${item.outTime}" /></td>
							<td>${item.nextNo}</td>
