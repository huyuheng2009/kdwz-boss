    ...
    <script type="text/javascript">
    	...
    	function save(flag){
    		...
    		$.ajax({
    			type: "POST",
				url: "${ctx}/notice/sadd",
    			data: "id="+id+"&title="+title+"&content="+content+"&isRedTitle="+isRedTitle+"&isSend="+flag,
...
<body>
   <div class="main_div">
   		<ul>
   		   <li class="left">标题</li>
   		    <li class="left"><input type="hidden" name="id" id="id" value="${notice.id }"/></li>
   		   <li class="left"><input type="text" name="title" id="title" value="${notice.title }"/></li>
   		...
   		<ul>
   		   ...
   		   <c:if test="${notice.isRedTitle==1 }">
   		   	 <input type="checkbox" name="isRedTitle" id="isRedTitle" value="1" checked="checked"/>
   		   </c:if>
   		   <c:if test="${notice.isRedTitle==0 }">
   		   	 <input type="checkbox" name="isRedTitle" id="isRedTitle" value="1"/>
   		   </c:if>
   		   
   		   <label for="isRedTitle">标题显示为红色</label></li>
   		   <li class="clear_fix"></li>
   		</ul>
   		<ul>
   		   <li> <textarea rows="20" cols="90" name="content" id="content" resize="no" placeholder="请输入60个字符以内">${notice.content }</textarea></li>
   		</ul>
   		<ul class="foot">
