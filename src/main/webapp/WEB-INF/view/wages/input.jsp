                              ...
                              $jqq("#courierNo").autocomplete(availablesrcKey1, {
                           		...
                           		formatItem: function(data11, i, total) {
                         			 ...
                         			 $("#courierNo").val(data[0].substring(0,data[0].indexOf('('))) ;
            	           			 $("#cname").val(data[0].substring(data[0].indexOf('(')+1,data[0].indexOf(')')));
                         	});	 
               }); 	      
           			 ...
           			 console.log(fdata);
           	  		 $.ajax({
           	  			 type: "post",//使用get方法访问后台
           	  	          ...
           	  	          url: "${ctx }/wages/inputSave",//要访问的后台地址
           		            data: fdata,//要发送的数据
           	  	            success: function(data){//msg为返回的数据，在这里做数据绑定
           	  	            	if(data=='1'){
           	  	            	   $('input:text').val("");
           	  	            	   $('input[name=dateMonth]').val(dateMonth) ;
           	  	            	   $('input[name=courierNo]').focus() ;
           	  	            	}else{
           	  	            		alert(data) ;
           	  	            	}
...
<div class="online_cont">
  <div class="shoujian_cont salary_head">
    <div class="shou_li"><strong><b></b>费用产生时间</strong><input type="text" class="sinput01 validate[required]"    name="dateMonth" value="${params['dateMonth']}" onClick="WdatePicker({dateFmt:'yyyy-MM'})"/></div>
    <div class="shou_li"><strong><b></b>快递员</strong><input class="validate[required]" type="text" style="width: 250px;" name="courierNo" id="courierNo" />
    <input type="text" class="sinput02" disabled="disabled" value="" name="cname" id="cname" /></div>
  </div>
  <div class="salary_cont">
    <div class="salary_cli left">
     <div class="salary_tit">工资增加项</div>
     	 ...
     	 <div class="shou_li"><strong><b></b>${item0.cn_name}</strong><input name="${item0.name}" type="text" class="validate[funcCall[xiaoshu]] "/>元</div>
        </c:forEach> 
   ...
   <c:forEach items="${outList}" var="item1" varStatus="status1">     
     	 <div class="shou_li"><strong><b></b>${item1.cn_name}</strong><input name="${item1.name}" type="text" class="validate[funcCall[xiaoshu]] ${status1.count==fn:length(outList)? 'submit ' : ''}" />元</div>
      </c:forEach> 
    </div>
  </div>
