...
<script type="text/javascript" src="/scripts/jquery.autocomplete.js"></script>
    ...
    function commit(id){
   	 $.ajax({type: "post",url:'/gather/commit',data:{'ids':id,'examineStatus':'3','pass':'1'},success:function(msg){
			 if(msg==1){
				 alert('审核成功');
				 location.reload();
			 }else{
...
<div class="tableble_search">
    ...
    <form:form id="trans" action="${ctx}/gather/mexam" method="get">
...
<div class="tbdata">
    ...
    <table width="100%" cellspacing="0" class="t2" id="t2">
        ...
        <c:forEach items="${list.list}" var="item" varStatus="status">
         <c:set var="sum_mcount" value="${sum_mcount+item.mcount}"></c:set>
            <tr class="${status.count % 2 == 0 ? 'a1' : ''}">
                ...
                <td>${item.mphome}</td>
                <td>${item.memail}</td>
                 <td> <c:if test="${item.liced eq 'Y'}">是</c:if>
                     <c:if test="${item.liced ne 'Y'}">否</c:if></td>
                <td>${item.note}</td>
        ...
        <tr class="${status.count % 2 == 0 ? 'a1' : ''}" style="background: #f5f5f5;border: none;" >
                <td align="center" colspan="9" style="background: #f5f5f5;border: none;"></td>
                <td style="background: #f5f5f5;border: none;"> 合计：${sum_mcount }</td>
                <td style="background: #f5f5f5;border: none;" colspan="12"></td>
            </tr>
        </tbody>
    </table>
</div>
