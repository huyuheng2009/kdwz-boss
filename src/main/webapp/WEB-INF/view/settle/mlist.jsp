...
.tableble_table tr.el td{
    ...
    function batch_pay(ids){
    	 ...
    	 $.dialog({lock: true,title:'批量收款',drag: true,width:630,height:600,resize: false,max: false,data:{ids:ids},content: 'url:${ctx}/settle/mpayPage?len='+len+'&id='+id+'&layout=no',close: function(){
       	 }});
    }
   	 ...
   	 if(confirm("确定取消付款记录吗？")){
       	 $.ajax({
			 type: "post",//使用get方法访问后台
	           ...
	           url: "${ctx }/settle/mbatchCancel",//要访问的后台地址
	            data: {'ids':ids},//要发送的数据
          ...
          $jqq("#monthSettleNo").autocomplete(mres, {
            ...
            $('#batch_pay').click(function(){
            	...
            	$('.ids').each(function(){
            	 ...
            	 $.dialog({lock: true,title:'批量收款',drag: true,width:630,height:400,resize: false,max: false,data:{ids:ids},content: 'url:${ctx}/settle/mpayPage?len='+len+'&id='+id+'&layout=no',close: function(){
               	 }});
...
<div class="search">
    ...
    <form:form id="trans" action="${ctx}/settle/mlist" method="post">
   ...
   <table width="100%" cellspacing="0" class="t2" id="t2">
            ...
            <c:set var="sum_settle_count" value="0"></c:set>
                ...
                <c:set var="sum_settle_count" value="${sum_settle_count+item.settle_count}"></c:set>
					...
					<td align="center"><input  class="ids" name="ids" <c:out value="${item.examine_status eq '1'?'disabled':''}" />  type="checkbox"  value="${item.id}" /></td>
