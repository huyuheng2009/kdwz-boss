...
.note_c {
    ...
    <script type="text/javascript">
        $(function () {
     	   ...
     	   var len =   $.trim($("input[name=len]").val());
     	    ...
     	    if(len=='0'){
    			 ...
    			 type: "post",//使用get方法访问后台
    	           ...
    	           url: "${ctx }/settle/batchPay",//要访问的后台地址
    	            data: {'ids':ids,'len':len,'settleType':settleType,'settleCount':settleCount,'note':note},//要发送的数据
...
<input type="hidden" name="len" value="${params.len}"/>
 <ul>
   ...
   <c:if test="${params.len eq '1'}">
   <li style="width: 100%;margin-bottom: 20px;"><span style="width: 100px;float: left;text-align: right;">实收金额：</span><input type="text" style="width: 378px;" disabled="disabled" name="settleCount"  placeholder="批量收款实收金额等于应收"  /></li>
   </c:if>
     <c:if test="${params.len eq '0'}">
