...
<script type="text/javascript" src="/scripts/jquery.autocomplete.js"></script>
   ...
   function showAddr(e){
	...
	function edit(id){
		$.dialog({lock: true,title:'编辑报价规则',drag: true,width:850,height:950,resize: false,max: false,content: 'url:${ctx}/franchise/redit?id='+id+'&layout=no',close: function(){
    		//location.reload();
    	 }});
	}
 	...
 	function del(id){
        ...
        $(function () {
        	...
        	$('#plxg').click(function(){
        		...
        		$.dialog({lock: true,title:'批量修改',drag: true,width:450,height:400,resize: false,max: false,content: 'url:${ctx}/franOrder/batchUpdate?ids='+ids+'&layout=no',close: function(){
            		//location.reload();
            	 }});
        	});
...
<div class="search">
    ...
    <form:form id="trans" action="${ctx}/franOrder/zlist" method="post">
     <input type="hidden" name="serviceName" value="jmzlist"/>
     <input type="hidden" name="ff" value="${params['ff']}"/>
        <ul>
        	<textarea  cols="18" rows="4"   maxlength="1400" placeholder="请输入运单号" name="lgcOrderNo" class="order_area" >${params['lgcOrderNo']}</textarea> 
		     <li><select name="ttype" style="width: 110px">
                 ...
                 <option value="PPP" ${params['ttype'] eq 'PPP'?'selected':''}>批带时间</option>
             </select></li>  
		   <li>
		...
		<li>取件员：<input type="text"  value="${params['tcourierNo']}" name="tcourierNo" id="tcourierNo" style="width: 200px;"></input></li>
  ...
  <div class="tableble_search ">
        <div class="operator">
          <div class="search_new"><input class="sear_butt" type="submit" id="pidai" value="批带" /></div>
          <div class="search_new"><input class="sear_butt" type="submit" id="plxg" value="批量修改" /></div>
...
<div class="tbdata">
    <table width="100%" cellspacing="0" class="t2" id="t2">
        ...
        <c:forEach items="${list.list}" var="item" varStatus="status">
            <tr>
            <td align="center"><input  class="ids" name="ids" type="checkbox"  <c:out value="${item.confirm_status eq 'SUCCESS'?'disabled':''}" />  value="${item.id}" /></td>
            <td>${status.count}</td>
