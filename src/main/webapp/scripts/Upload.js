function Upload($e, accepts,url) {
	...
	if (accepts.length > 0) {
		...
		this.$file.attr("accept", accepts);
	}
	if (arguments.length > 3) {
		this.opts = arguments[3];
	} else {
		this.opts = {};
	}
	//
...
Upload.prototype.accept = function(accepts) {
	this.accepts = accepts;
	if (accepts.length > 0) {
		this.$file.attr("accept", accepts);
	}
...
Upload.prototype.setParams = function(params) {
	this.opts = params;
}

Upload.prototype.uploadUrl = function(url) {
	this.url = url ;
}

Upload.prototype.process = function(process) {
	if (process >= 0) {
		...
		switch (process) {
			...
			case -100:
				...
				console.log(ret)
				if (ret.errorCode === 0) {
					this.$e.find(".p2").addClass("p2_green");
					if(ret.message){
						this.$e.find(".a2").text(ret.message);
					}else{
...
Upload.prototype.do = function(evt) {
	if (this.lock) {
		return;
	}
	...
	this.$e.find(".p2").removeClass("p2_green");
	//
	var _this = this;
	this.lock = true;
	var status = 0;
		...
		if (this.readyState === this.DONE) {
			if (this.status !== 200) {
				...
				if (this.response !== null){
					...
					res.errorCode=parseInt(res.errorCode+"");
				}
