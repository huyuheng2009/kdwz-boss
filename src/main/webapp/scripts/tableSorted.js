...
{
TableSorter.prototype.lastLen = 0 ;
this.Table = this.$(table);
if(this.Table.rows[0].cells[0].className.indexOf("sumc")>-1){
	TableSorter.prototype.lastLen = 1 ;
}
...
{
	if(this.Table.rows[0].cells[args[x]].className.indexOf("nosort")>0){
		continue ;
	}else{
...
for(var x = 0; x < this.Table.rows[0].cells.length; x++)
{
	if(this.Table.rows[0].cells[x].className.indexOf("nosort")>-1){
		continue ;
	}else{
...
var Sorter = [];
for(var x = 1; x < this.Table.rows.length-this.lastLen; x++)
{
...
//可自行实现排序后的动作.
TableSorter.prototype.OnSorted = function()
{
	 var el =  document.getElementsByClassName("select_bg") ;
		for(var i=0;i<el.length;i++){
		      el[i].className ='select_bg';  
	  }
	for(var j=0;j<this.lno.length;j++){
		//	this.$(this.lno[j]).parentNode.parentNode.className+=" el" ; 
	  }
    return; 
}
//可自行实现排序后的动作.
TableSorter.prototype.BeforeSort = function(cell, IsAsc)
{
	this.lno=[] ;
	var el =  document.getElementsByClassName("el") ;
	for(var i=0;i<el.length;i++){
		this.lno.push(el[i].getElementsByClassName("lgc_order_no")[0].id);
	}
   return;
}
