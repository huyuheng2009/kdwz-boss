...
{ 
	var id = data.id;
	var src = Web.attachment_dirname+data.file_path.replaceAll2Excep('\\','/');
	var ext =  1;
	...
	var datas='<div id="uplist_'+id+'"><input type="hidden" name="status" id="status" value="0"><input type="hidden"  id="ids" name="ids[]"  value="'+id+'"  /><input type="text"  id="filedata" name="filedata[]" value="'+src+'"  /> <input type="text" id="namedata" name="namedata[]" value="'+name+'"  /> &nbsp;<a href="javascript:remove_this(\'uplist_'+id+'\');">移除</a> </div>';
...
function add_file(obj,id,status){
	...
	if($(obj).hasClass('on')){
		...
		if(num < file_limit){
			$(obj).addClass("on");
			var datas='<div id="uplist_'+id+'"><input type="hidden" name="status" id="status" value="'+status+'"><input type="hidden"  id="ids" name="ids[]"  value="'+id+'"  /><input type="text"  id="filedata" name="filedata[]" value="'+src+'"  /> <input type="text" id="namedata" name="namedata[]" value="'+name+'"  /> &nbsp;<a href="javascript:remove_this(\'uplist_'+id+'\');">移除</a> </div>';
			$('#myuploadform').append(datas);		
			ids.push(id);
		}else{
			alert('已经达到附件限制数');
...
function uploadSuccess(file, serverData) {
	try {		
		...
		if (serverData.flag)			
        {					
			progress.setStatus(serverData.prompt);
...
function uploadComplete(file) {
	...
	if (this.getStats().files_queued === 0) {
		//alert(this.customSettings.cancelButtonId);
