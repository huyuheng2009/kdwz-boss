...
*/
   ...
   dhtmlXTreeObject.prototype.setCheckStr=function(itemStr,state){
	   
	     var sNode;
		 var itemArr = itemStr.split(",")
		 for (var i=0 ; i<itemArr.length;i++){
		 	   sNode = this._globalIdStorageFind(itemArr[i],0,1);
		 	  if (!sNode) {
		 		  return;
 ...
 *     @desc: 选中复选框
 *     @type: public
 *     @param: itemStr - target node id string , format:1,2,3,4
 *     @param: state - checkbox state 0:
 *     @topic: 5
 */
dhtmlXTreeObject.prototype.setSubCheckedStr=function(itemStr,state){
	 var sNode;
	 var itemArr = itemStr.split(",")
	 for (var i=0 ; i<itemArr.length;i++){
	 	   sNode = this._globalIdStorageFind(itemArr[i]);
	 	if(sNode!=null){
	   		 this._setSubChecked(state,sNode);
	   		 this._correctCheckStates(sNode.parentObject);
	   	}	 
